...
public class Turret extends SubsystemBase {
    ...
    public enum AimTarget {
        ...
        HUB(new Translation3d(4.5,Constants.Field.kFieldWidth/2,1.88),0, 50),
    ...
    public double getRotationToTarget(AimTarget targetPosition) {
        ...
        return getClosestLegalRotationDegrees(rawRotationToTargetDegrees);
    }
    ...
    private double getAllianceRobotAngularVelocityRadiansPerSecond() {
        double angularVelocity = robotAngularVelocitySupplier.getAsDouble();
        return SwerveDrive.getAlliance() == edu.wpi.first.wpilibj.DriverStation.Alliance.Red
            ? -angularVelocity
            : angularVelocity;
    }

    private Translation2d getAllianceTurretVelocity(Translation2d robotVelocity, double yawRadians) {
        Translation2d turretOffset = new Translation2d(
            offsetTranslation.getX() * Math.cos(yawRadians) - offsetTranslation.getY() * Math.sin(yawRadians),
            offsetTranslation.getX() * Math.sin(yawRadians) + offsetTranslation.getY() * Math.cos(yawRadians));
        double angularVelocity = getAllianceRobotAngularVelocityRadiansPerSecond();
        Translation2d tangentialVelocity = new Translation2d(
            -angularVelocity * turretOffset.getY(),
            angularVelocity * turretOffset.getX());
        return robotVelocity.plus(tangentialVelocity);
    }
    ...
    private double getClosestLegalRotationDegrees(double rawRotationDegrees) {
        double currentRotationDegrees = getRotationDegrees();
        double minRotationBound = Math.min(maxRotationClockwise, maxRotationCounterclockwise);
        double maxRotationBound = Math.max(maxRotationClockwise, maxRotationCounterclockwise);

        double bestLegalRotation = MathUtil.clamp(rawRotationDegrees, minRotationBound, maxRotationBound);
        ...
        for (int wraps = -2; wraps <= 2; wraps++) {
            ...
            double candidateError = Math.abs(candidateRotation - currentRotationDegrees);
            if (candidateError < bestError) {
                bestError = candidateError;
                bestLegalRotation = candidateRotation;
            }
        }
    ...
    private double getMovingLeadScale(
        ...
        double midLowLatScale = Constants.tunableNumber(
            Constants.Turret.SWIM.kLeadScaleMidLowLatKey,
            Constants.Turret.SWIM.kLeadScaleMidLowLatDefault);
        double midHighLatScale = Constants.tunableNumber(
            Constants.Turret.SWIM.kLeadScaleMidHighLatKey,
            Constants.Turret.SWIM.kLeadScaleMidHighLatDefault);
        double farLowLatScale = Constants.tunableNumber(
            Constants.Turret.SWIM.kLeadScaleFarLowLatKey,
            Constants.Turret.SWIM.kLeadScaleFarLowLatDefault);
        double farHighLatScale = Constants.tunableNumber(
            Constants.Turret.SWIM.kLeadScaleFarHighLatKey,
            Constants.Turret.SWIM.kLeadScaleFarHighLatDefault);

        double midLateralBlend = MathUtil.clamp(
            lateralVelocity / Constants.Turret.SWIM.kLeadLowLateralSpeed,
            0.0,
            1.0);
        double farLateralBlend = MathUtil.clamp(
            lateralVelocity / Constants.Turret.SWIM.kLeadHighLateralSpeed,
            0.0,
            1.0);

        double midScale = MathUtil.interpolate(midLowLatScale, midHighLatScale, midLateralBlend);
        double farScale = MathUtil.interpolate(farLowLatScale, farHighLatScale, farLateralBlend);

        if (rangeMeters <= Constants.Turret.SWIM.kLeadNearRangeMeters) {
            return nearScale;
        }

        if (rangeMeters <= Constants.Turret.SWIM.kLeadMidRangeMeters) {
            double rangeBlend = MathUtil.clamp(
                (rangeMeters - Constants.Turret.SWIM.kLeadNearRangeMeters)
                    / (Constants.Turret.SWIM.kLeadMidRangeMeters - Constants.Turret.SWIM.kLeadNearRangeMeters),
                0.0,
                1.0);
            return MathUtil.interpolate(nearScale, midScale, rangeBlend);
        }

        double farRangeBlend = MathUtil.clamp(
            (rangeMeters - Constants.Turret.SWIM.kLeadMidRangeMeters)
                / (Constants.Turret.SWIM.kLeadMidRangeMeters - Constants.Turret.SWIM.kLeadNearRangeMeters),
            0.0,
            1.0);
        return MathUtil.interpolate(midScale, farScale, farRangeBlend);
    }
